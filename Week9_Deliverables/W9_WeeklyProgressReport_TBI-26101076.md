...
| `REACT_APP_API_URL` undefined in build | No `.env.production` file + Vercel env var missing | Created `.env.production` AND set var via Vercel CLI |
| "Route not found" on login | `express-rate-limit` crashing behind Render reverse proxy | Added `app.set('trust proxy', 1)` to `server.js` |
| Groq API key header error | Render dashboard added hidden newline when key was pasted | Added `.trim()` to `GROQ_API_KEY` in `groq.service.js` |
| All reviews returning Neutral | Groq API key missing on Render + local NLP word list too small | Set `GROQ_API_KEY` on Render, expanded NLP word list, weighted negative scoring |
| Mixed reviews returning Negative | Groq prompt too decisive, no neutral concept | Updated system prompt to classify mixed reviews as Neutral |
| HuggingFace DNS failure on Render | Render free tier blocks external DNS for HF endpoint | Removed HF from sentiment chain, reduced timeout to 5s for analysis service |
| Google OAuth COOP warning | Vercel domain not in Firebase authorized domains | Added `stay-inn-sight-ai-f3ov.vercel.app` to Firebase authorized domains |
...
| File | Change |
|------|--------|
| `backend/server.js` | Dynamic CORS, `/api/health` endpoint, keep-alive self-ping, `trust proxy` |
| `backend/.env.example` | Added `FRONTEND_URL`, `NODE_ENV` documentation |
| `backend/services/groq.service.js` | `.trim()` on API key to prevent header errors |
| `backend/services/sentiment.service.js` | Expanded word list, weighted scoring, updated Groq prompt for neutral detection |
| `backend/services/analysis.service.js` | Updated analysis prompt with explicit neutral rules for mixed reviews |
| `backend/services/huggingface.service.js` | Reduced timeout to 5s for fast-fail on Render |
